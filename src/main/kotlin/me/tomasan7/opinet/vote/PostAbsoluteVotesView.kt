...
/*
SELECT POST.ID, SUM(CASE WHEN VOTE.UP_DOWN THEN 1 WHEN NOT VOTE.UP_DOWN THEN -1 ELSE 0 END) AS votes
FROM POST
...
private val VOTE_SUM_EXPR = Expression.Companion.build {
    case()
        .When(VoteTable.upDown, intLiteral(1))
        .When(not(VoteTable.upDown), intLiteral(-1))
        .Else(intLiteral(0))
        .sum()
